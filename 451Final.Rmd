...
We vaguely get a sense that both world ODA and U.S. ODA positively contribute to GDP total growth rate. Also, we would want to cut U.S ODA into three categories: 0~250, 251~500, 500+ because we can see that there seem to be an increasing trend between 0-250, and another decreasing trend between 250 and 500. 
...
```{r}
mod1 <- lm(GDP_total_growth_rate~ as.factor(US_ODA_C) +ns(recipient_trade_world,2)+ns(polity2,3)+ns(population_growth_rate,2),data = aid2012)
summary(mod1)
confint(mod1)
```
...
mod_A <- lm(US_ODA ~ ns(polity2,3)+recipient_trade_world, data = aid2007)
mod_Y <- lm(GDP_total_growth_rate ~US_ODA +recipient_trade_world+ns(polity2,3)+ns(population_growth_rate,3), data = aid2007)
...
U_A_assocs <- seq(from = 1, to = sigma(mod_A)/10, length.out = 10)
U_Y_assocs <- seq(from = -5, to = -1, by = 1)
