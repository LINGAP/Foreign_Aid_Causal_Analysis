...
Now, let's visualize the distribution of the missing data.
...
- alpha = 0: holding an extremely high bar for an estimate of ACE to be counted
as significant. No edge will be present in the resulted graph.
- alpha = 1: any relationship between nodes is considered to be valid and real.
The resulted graph skeleton will be strongly connected.
...
Informed by rtheg causal discovery algorithm, we also explore the relationship between US_ODA and other variables. In the below section, we visualize the relationships that are suggested by the causal discovery algorithm.
...
Before we construct the regression model, we would also want to visualize the association between U.S aid and other controlled variables in our model. The goal of visualization is to find out if the association is linear or not and if we should introduce the nonlinearity into our model.

### GDP growth rate vs. US foreign aid (Treatment vs. Outcome)

```{r}
ggplot(data = aid2007,aes(x = GDP_total_growth_rate,y = log(US_ODA)))+
  geom_point()+
  geom_smooth()
```
...
```{r}
ggplot(data = aid2007, aes(x = population_growth_rate, y = log(US_ODA))) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
...
```{r}
ggplot(data = aid2007, aes(x = log(recipient_trade_world), y = log(US_ODA))) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,2), method = "lm",
    method.args = list(family="binomial"),
...
```{r}
ggplot(data = aid2007, aes(x = polity2, y = log(US_ODA))) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,3), method = "lm",
    method.args = list(family="binomial"),
...
In this graph, we see that association between U.S ODA and politics score is non-linear; therefore, this variable should also be non-linear in our model

##### 4. Does polity2 interact with US_ODA?

When alpha is relaxed to 0.12, both polity2 and US_ODA appear to affect the growth outcome directly. Therefore, we wonder if the countries would have different growth given different polity2 even they receive the same amount of US_ODA.
...
```{r}
mod<-lm(GDP_total_growth_rate~ US_ODA, data = aid2007)
summary(mod)
confint(mod)
```

```{r}
mod1 <- lm(GDP_total_growth_rate~ ns(US_ODA,3),data = aid2007)
summary(mod1)
confint(mod1)
```
...
```{r}
mod1 <- lm(GDP_total_growth_rate~ US_ODA +ns(recipient_trade_world,2)+ns(polity2,3)+ns(population_growth_rate,3),data = aid2007)
summary(mod1)
confint(mod1)
```

According to our model, the coefficient of U.S aid is 0.0001812 , suggesting that U.S ODA can generate positive economic impact on the recipients; however, the impact is relatively small and not significant in 2007. Also, we can see that the coefficients of U.S ODA is not statistically significant after we incorporate all the controlled variables in our model. Therefore, we might suggest that there are indirect paths connecting the U.S ODA and the GDP growth rate of recipient countries; and we might conclude that U.S ODA can contribute to the GDP growth rate of recipients countries indirectly, but we still can not verify the vadility of the direct path or the direct impact of U.S ODA and the GDP growth rate of recipient countries.
...
```{r}
p1 <- ggplot(data = aid2007, aes(x = population_growth_rate, y = log(US_aid_total))) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,3), method = "lm",
    method.args = list(family="binomial"),
...
```{r}
p2 <- ggplot(data = aid2007, aes(x = log(recipient_trade_world), y = log(US_aid_total))) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
...
```{r}
p3 <- ggplot(data = aid2007, aes(x = polity2, y = log(US_aid_total))) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,3), method = "lm",
    method.args = list(family="binomial"),
...
```{r}
mod2 <- lm(GDP_total_growth_rate~ US_ODA +recipient_trade_world+ns(polity2,3)+ns(population_growth_rate,3),data = aid2007)
summary(mod2)
confint(mod2)
```

The coefficient of the U.S aid is positive but not significant, which is consistent with our previous results. 
...
sensitivity_analysis <- function(.data, model_A, model_Y, assoc_A, assoc_Y) {
    ...
    updated_model <- lm(GDP_total_growth_rate ~US_ODA +recipient_trade_world+ns(polity2,3)+ns(population_growth_rate,3)+U, data = .data)
...
mod_Y <- lm(GDP_total_growth_rate~US_ODA +ns(recipient_trade_world,2)+ns(polity2,3)+ns(population_growth_rate,3), data = aid2007)
...
U_A_assocs <- seq(from = 0.01, to = sigma(mod_A)/10, length.out = 10)
U_Y_assocs <- seq(from = -sigma(mod_Y), to = -0.01, by = 0.5)
...
```{r}
p4<- ggplot(data = aid2012, aes(x = population_growth_rate, y = log(US_ODA))) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,2), method = "lm",
    method.args = list(family="binomial"),
...
```{r}
p5<- ggplot(data = aid2012, aes(x = log(recipient_trade_world), y = log(US_ODA))) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,1), method = "lm",
    method.args = list(family="binomial"),
...
```{r}
p6<- ggplot(data = aid2012, aes(x = polity2, y = log(US_ODA))) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,3), method = "lm",
    method.args = list(family="binomial"),
...
```{r}
mod3 <- lm(GDP_total_growth_rate~ US_ODA +recipient_trade_world+ns(polity2,3)+ns(population_growth_rate,2),data = aid2012)
summary(mod3)
confint(mod3)
```

The coefficient of U.S ODA is positive but not significant in 2012; therefore, our results are consistent with using data from 2007. 


## Another Measurement of Economic growth

```{r}
mod4 <- lm(GDP_per_growth_rate ~ US_ODA +ns(recipient_trade_world,2)+ns(polity2,3)+ns(population_growth_rate,3),data = aid2007)
summary(mod4)
confint(mod4)
```

We also switch to another common estimator of economic growth: GDP per capita growth rate. The coefficient of U.S ODA is still not significant and the magnitude remains small; therefore, our results are consistent with the previous regression model. 
