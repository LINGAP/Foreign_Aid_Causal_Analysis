...
The core idea of the algorithm is that if conditional on all possible sets of other variables, there remains a significant relationship between variable A and B, then there exists a direct causal association between them. The direction of the arrow is determined by controlling 
...
Informed by the causal discovery algorithm, we also explore the relationship between US_ODA and other variables. In the below section, we visualize the relationships that are suggested by the causal discovery algorithm.
...
We vaguely get a sense that both world ODA and U.S. ODA positively contribute to GDP total growth rate. Also, we would want to cut U.S ODA into two categories: [0,250] and [251,+) because we can see that there seem to be an increasing trend between 0-250, and another decreasing trend for US_ODA that's larger than 250. 

```{r}
aid2012 <- aid2012%>%
  mutate(US_ODA_C = ifelse(US_ODA<250, 0,1))
aid2012$US_ODA_C = as.factor(aid2012$US_ODA_C)
...
## Choice of linearity of other variables 

Before we construct the regression model, we would first want to visualize the association between U.S aid and other controlled variables in our model. The goal of visualization is to find out if the association is linear or not and if we should introduce the nonlinearity into our model. For all graphs below, blue lines are dervied from the actual data, providing insights on how a variable affects if one receives the treatment or not. The red lines are used to depict that relationship.


##### 1. Population growth rate vs. U.S ODA

```{r}
ggplot(data = aid2012, aes(x = population_growth_rate, y = US_ODA_C))+
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
...
##### 2. Trade balance of recipient countries vs. U.S ODA

```{r}
ggplot(data = aid2012, aes(x = recipient_trade_world, y = US_ODA_C)) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,2), method = "glm",
    method.args = list(family="binomial"),
...
```{r}
mod<-lm(GDP_total_growth_rate~ US_ODA_C, data = aid2012)
summary(mod)
confint(mod)
```
...
```{r}
mod1 <- lm(GDP_total_growth_rate~ US_ODA_C +ns(recipient_trade_world,2)+ns(polity2,3)+ns(population_growth_rate,2),data = aid2012)
summary(mod1)
confint(mod1)
```

According to our model, if the U.S ODA is smaller than 250, then it is expected to contribute to 1.4262 percent GDP growth for the recipient countries in 2012 when we hold other variables as constant; however, if the U.S ODA is greater than 250, then the expected GDP growth rate would only increase by 0.6577 percent in 2012 holding other variables constant. Yet, the benefits of U.S ODA is insignificant no matter it is greater or less than 250 in 2012.
...
According to the graph, we might still want to cut thhe U.S aid total at around 250 because there is an increasing trend between 0 and 250. 
```{r}
aid2012 <- aid2012%>%
...
```{r}
p1 <- ggplot(data = aid2012, aes(x = population_growth_rate, y = US_aid_total_C )) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
...
```{r}
p2 <- ggplot(data = aid2012, aes(x =recipient_trade_world, y = US_aid_total_C )) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,1), method = "glm",
    method.args = list(family="binomial"),
    se = FALSE, color = "red" )
```
...
```{r}
p3 <- ggplot(data = aid2012, aes(x = polity2, y = US_aid_total_C )) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,3), method = "glm",
    method.args = list(family="binomial"),
...
```{r}
mod2 <- lm(GDP_total_growth_rate~ as.factor(US_aid_total_C) +recipient_trade_world+ns(polity2,3)+ns(population_growth_rate,3),data = aid2012)
summary(mod2)
confint(mod2)
```

As we switch to using the total amount of U.S aid, we can see that when the total aid is smaller than 250, the GDP growth rate would be expected to increaseby 4.7772 percent holding all else constant in 2007; however, if the aid is greater than 250, the GDP growth rate would be expected to increase by 4.1737 percent holding all else constant in 2007. Yet, both benefits are insignificant. The results are consistent with our findings that use U.S ODA as the treatment. 
...
sensitivity_analysis <- function(.data, model_A, model_Y, assoc_A, assoc_Y) {
    ...
    updated_model <- lm(GDP_total_growth_rate~ as.factor(US_ODA_C) +ns(recipient_trade_world,2)+ns(polity2,3)+ns(population_growth_rate,2)+U, data = .data)
...
mod_A <- lm(as.factor(US_ODA_C) ~ ns(polity2,3)+recipient_trade_world, data = aid2007)
mod_Y <- lm(GDP_total_growth_rate~ as.factor(US_ODA_C) +ns(recipient_trade_world,2)+ns(polity2,3)+ns(population_growth_rate,2), data = aid2007)
...
U_A_assocs <- seq(from = 1, to = sigma(mod_A)/20, length.out = 10)
U_Y_assocs <- seq(from = -sigma(mod_Y), to = -0.01, by = 1)
...
```{r}
p4<- ggplot(data = aid2007, aes(x = population_growth_rate, y = US_ODA_C)) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,3), method = "glm",
    method.args = list(family="binomial"),
...
```{r}
p5<- ggplot(data = aid2012, aes(x = recipient_trade_world, y = US_ODA_C)) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,2), method = "glm",
    method.args = list(family="binomial"),
...
```{r}
p6<- ggplot(data = aid2012, aes(x = polity2, y = US_ODA_C)) +
  geom_point() +
  geom_smooth(se = FALSE, color = "blue")+ 
  geom_smooth(formula = y~ns(x,3), method = "glm",
    method.args = list(family="binomial"),
...
```{r}
mod3 <- lm(GDP_total_growth_rate~ as.factor(US_ODA_C) +ns(recipient_trade_world,2)+ns(polity2,3)+ns(population_growth_rate,3),data = aid2007)
summary(mod3)
confint(mod3)
```

When we use the data from 2007, we can see that if the U.S ODA is smaller than 100, the GDP growth rate is expected to increase by 12.3425 percent while we holding other variables as constant; while if the U.S ODA is greater than 100, the GDP growth rate of recipients would be expected to increase by 13.6954 percent holding other variable constant. However, the benefits of U.S ODA (<100) is significant, which is different from our previous results using data from 2012. Also, the additional benefit of larger U.S ODA is also not significant; therefore, we can conclude that the more the U.S ODA, the greater economic impact it can generate. 
...
```{r}
mod4 <- lm(GDP_per_growth_rate ~ as.factor(US_ODA_C) +ns(recipient_trade_world,2)+ns(polity2,3)+ns(population_growth_rate,3),data = aid2012)
summary(mod4)
confint(mod4)
```

We also switch to another common estimator of economic growth: GDP per capita growth rate. The coefficient of U.S ODA is still not significant and the magnitude remains small; therefore, our results are consistent with the previous regression model using GDP growth rate in 2012. 
