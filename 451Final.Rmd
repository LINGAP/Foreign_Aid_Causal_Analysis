...
```{r}
ggplot(data = aid2007,aes(y = GDP_total_growth_rate,x = US_aid_total))+
  geom_smooth()
```
...
```{r}
ggplot(aid2007, aes(x = US_aid_total, y = GDP_total_growth_rate, color = population_growth_rate)) + 
  geom_point()
```
...
## Regression Model
```{r}
mod <- lm(GDP_per_growth_rate~US_aid_total + oda_gni+recipient_trade_world+polity2+population_growth_rate+Rest_of_world_aid_total,data = aid2007)
summary(mod)
confint(mod)
```
